...
class UserController extends BaseController
{
    ...
    public function doCheckIn($request, $response, $args)
    {
        if (!$this->user->isAbleToCheckin()) {
		 ...
		 if (!$this->user->isMFirstCheckin())
		 {
			 if (($this->user->level_number) >= 3 ) {
			$traffic = rand(Config::get('checkinMin'), Config::get('checkinMax'));
			$Extraffic = rand(1024, 2048);
			$trafficToAdd = Tools::toMB($traffic) + Tools::toMB($Extraffic);
			...
			$res['msg'] = sprintf("获得了 %u MB流量,通过等级特权获取 %u MB流量.", $traffic , $Extraffic);
			$res['ret'] = 1;
			return $this->echoJson($response, $res);
			}
        $traffic = rand(Config::get('checkinMin'), Config::get('checkinMax'));
		...
		if (($this->user->level_number) >= 3 ) {
			$traffic = rand(Config::get('checkinMin'), Config::get('checkinMax'));
			$Extraffic = rand(1024, 2048);
			$trafficToAdd = Tools::toMB($traffic) + Tools::toMB($Extraffic) + Tools::toMB(20480);
			$this->user->transfer_enable = $this->user->transfer_enable + $trafficToAdd;
			$this->user->last_check_in_time = time();
			$this->user->save();
			// checkin log
			...
			$res['msg'] = sprintf("获得了 %u MB流量,通过等级特权获取 %u MB流量,每月固定赠送 20480 MB流量.", $traffic , $Extraffic);
			$res['ret'] = 1;
			return $this->echoJson($response, $res);
			}
			if (($this->user->level_number) >= 2 ){
        $traffic = rand(Config::get('checkinMin'), Config::get('checkinMax'));
        $trafficToAdd = Tools::toMB($traffic) + Tools::toMB(20480);
        $this->user->transfer_enable = $this->user->transfer_enable + $trafficToAdd;
        $this->user->last_check_in_time = time();
        $this->user->save();
        // checkin log
        ...
        $res['msg'] = sprintf("获得了 %u MB流量,每月固定赠送 20480 MB流量.", $traffic);
        $res['ret'] = 1;
        return $this->echoJson($response, $res);
		}
	        $traffic = rand(Config::get('checkinMin'), Config::get('checkinMax'));
